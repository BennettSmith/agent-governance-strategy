...
- **README improvements**
  ...
  - Clarify **pinning**:
    ...
    - governance content pin (`source.ref` like `gov/vX.Y.Z` or commit SHA)
...
- [x] `make ci`
- [x] `make fmt` updates Go + markdown formatting as expected (no unexpected file churn)
- [x] `make lint` lints markdown (and Go, if applicable) with clear failure output
- [x] Manual read-through: README “happy path” is clear and complete for a new reader
- [x] Links check (spot-check all new internal links and key external references)
